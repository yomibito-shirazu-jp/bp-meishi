...
export const getExecutiveAction = async (prompt: string | any[], modelName: string = 'gemini-3.1-flash-lite-preview', chatHistory?: any[]) => {
  const ai = getAI();
...
export const executeApprovedTool = async (call: any, chatHistory: any[]) => {
  ...
  }] as any, 'gemini-3.1-flash-lite-preview', chatHistory);
};
