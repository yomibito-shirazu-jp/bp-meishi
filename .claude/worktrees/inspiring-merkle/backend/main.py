...
# ── DTP Automation Endpoints ──

try:
    from backend.adobe_bridge import adobe as adobe_bridge
except ImportError:
    from adobe_bridge import adobe as adobe_bridge

class DtpPreviewRequest(BaseModel):
    pdf_b64: str
    page_index: int = 0
    dpi: int = 150

class DtpCorrectRequest(BaseModel):
    pdf_b64: str
    instructions: str
    page_indices: list[int] = []

class DtpExecuteRequest(BaseModel):
    operation: str
    files: list[dict] = []
    params: dict = {}

class DtpAdobeRequest(BaseModel):
    application: str          # photoshop | indesign | illustrator
    action: str
    options: dict = {}


@app.post("/dtp/preview")
async def dtp_preview(req: DtpPreviewRequest):
    """PDF プレビュー画像 + メタデータを返す"""
    try:
        pdf_bytes = base64.b64decode(req.pdf_b64)
        doc = fitz.open(stream=pdf_bytes, filetype="pdf")
        page_idx = min(req.page_index, len(doc) - 1)
        page = doc[page_idx]
        ...
        return {
            ...
            "pages": [
                {
                    "index": i,
                    "width_mm": round(doc[i].rect.width / 72 * 25.4, 1),
                    "height_mm": round(doc[i].rect.height / 72 * 25.4, 1),
                }
                for i in range(len(doc))
            ],
        }
    except Exception as e:
        raise HTTPException(500, f"プレビューエラー: {e}")


@app.post("/dtp/correct")
async def dtp_correct(req: DtpCorrectRequest):
    """AI 原稿修正: Gemini Vision で修正箇所を検出し構造化 JSON を返す"""
    if not GOOGLE_AI_KEY:
        raise HTTPException(400, "GOOGLE_AI_KEY が未設定です")

    try:
        pdf_bytes = base64.b64decode(req.pdf_b64)
        doc = fitz.open(stream=pdf_bytes, filetype="pdf")

        page_indices = req.page_indices or list(range(len(doc)))
        all_corrections = []
        ...
        for page_idx in page_indices:
            ...
            prompt = f"""あなたはDTP原稿修正AIです。以下の修正指示に基づいてこのPDFページ(ページ{page_idx + 1})の修正箇所を特定してください。
...
[{{
  "page": {page_idx},
  "location": "修正箇所の位置（例：右下の電話番号、ヘッダーのタイトル）",
  "original_text": "修正前のテキスト（画像から読み取った現在の値）",
  "corrected_text": "修正後のテキスト",
  "reason": "修正理由"
}}]

ルール:
- 修正指示に該当する箇所のみを返す（無関係な箇所は含めない）
- 該当箇所がなければ空配列 [] を返す
- original_text は画像から正確に読み取る
- corrected_text は修正指示に従って正確に記述する"""
            ...
            if res.status_code != 200:
                print(f"  Gemini correction error {res.status_code}: {res.text[:200]}")
                continue
            ...
            try:
                ...
                print(f"  Gemini returned non-JSON for page {page_idx}: {clean[:200]}")

        return {
            "corrections": all_corrections,
            "corrected_pdf_b64": None,  # 修正適用は別途 /rebuild で実施
        ...
        import traceback
        traceback.print_exc()
        raise HTTPException(500, f"AI修正エラー: {e}")


@app.post("/dtp/execute")
async def dtp_execute(req: DtpExecuteRequest):
    """DTP操作を実行 (PDF加工 or Adobe操作)"""
    try:
        op = req.operation
        files = req.files
        params = req.params
        ...
        # PDF操作 (PyMuPDF)
        if op == "pdf_merge" and len(files) >= 2:
            merged = fitz.open()
            for f in files:
                if f.get("data_b64"):
                    doc = fitz.open(stream=base64.b64decode(f["data_b64"]), filetype="pdf")
                    merged.insert_pdf(doc)
            buf = io.BytesIO()
            merged.save(buf, garbage=4, deflate=True)
            result_b64 = base64.b64encode(buf.getvalue()).decode()
            # Preview
            merged2 = fitz.open(stream=buf.getvalue(), filetype="pdf")
            pix = merged2[0].get_pixmap(matrix=fitz.Matrix(150/72, 150/72), alpha=False)
        ...
        elif op == "pdf_split" and files:
            pdf_bytes = base64.b64decode(files[0]["data_b64"])
            doc = fitz.open(stream=pdf_bytes, filetype="pdf")
            output_files = []
            for i in range(len(doc)):
                single = fitz.open()
                ...
                buf = io.BytesIO()
                single.save(buf, garbage=4, deflate=True)
                ...
                output_files.append({
                    ...
                    "preview_png_b64": base64.b64encode(pix.tobytes("png")).decode(),
                })
            return {"output_files": output_files}

        elif op == "pdf_compress" and files:
            pdf_bytes = base64.b64decode(files[0]["data_b64"])
            doc = fitz.open(stream=pdf_bytes, filetype="pdf")
            buf = io.BytesIO()
            ...
            return {
                "output_files": [{
                    "id": "compressed",
                    "name": f"compressed_{files[0].get('name', 'output.pdf')}",
                    "mime_type": "application/pdf",
                    "data_b64": result_b64,
                    "preview_png_b64": preview,
        ...
        # Adobe操作
        elif op in ("ps_edit", "id_layout", "ai_vector"):
            app_map = {"ps_edit": "photoshop", "id_layout": "indesign", "ai_vector": "illustrator"}
            application = app_map[op]
            instruction = params.get("instruction", "")

            # Adobe に送信
            result = adobe_bridge.send_command(application, "executeInstruction", {
                "instruction": instruction,
                "files": [{"name": f.get("name")} for f in files],
            })
            return {"output_files": [], "adobe_response": result}

        else:
            raise HTTPException(400, f"未対応の操作: {op}")
        ...
        import traceback
        traceback.print_exc()
        raise HTTPException(500, f"DTP実行エラー: {e}")


@app.post("/dtp/adobe")
async def dtp_adobe(req: DtpAdobeRequest):
    """Adobe アプリに直接コマンドを送信"""
    try:
        result = adobe_bridge.send_command(req.application, req.action, req.options)
...
def dtp_status():
    ...
    try:
        proxy_connected = adobe_bridge.check_connection()
    except Exception:
        pass
    return {
        "proxy_connected": proxy_connected,
        "proxy_url": adobe_bridge.proxy_url,
        "pymupdf": fitz.version[0],
        "gemini_configured": bool(GOOGLE_AI_KEY),
    }
