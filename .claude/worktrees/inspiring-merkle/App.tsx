...
import { Span, PageData, CardProject, AppState, TranscribeProject, AiResult, DtpTask, DtpFile, DtpCorrection, DtpOperationType } from './types';
...
import {
  ...
  FileAudio, Clock, List, Layers, Play, Check,
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const renderSidebar = () => {
    const sections = [
      ...
      },
      {
        title: '自動組版',
        items: [
          { icon: Layers, label: '一覧', badge: typesetTasks.filter(t => t.status === 'processing').length, state: AppState.TYPESET_LIST },
          { icon: Clock, label: '履歴', badge: 0, state: AppState.TYPESET_HISTORY },
  ...
  const renderHeader = () => (
    <div className="h-14 bg-white border-b px-6 flex items-center justify-between shrink-0" style={{ borderColor: C.border }}>
      <div className="flex items-center gap-4">
        ...
        {view === AppState.TYPESET_LIST && <h2 className="text-base font-bold text-slate-800">自動組版 一覧</h2>}
        {view === AppState.TYPESET_HISTORY && <h2 className="text-base font-bold text-slate-800">自動組版 履歴</h2>}
        {view === AppState.TYPESET_AI && (
          <div className="flex items-center gap-2">
            <Layers size={16} style={{ color: C.accent }} />
            <h2 className="text-base font-bold text-slate-800">AI組版</h2>
          </div>
        )}
        {view === AppState.EDIT && (
          <div className="flex items-center gap-2">
  ...
  // ── Typeset (自動組版) ──
  const [typesetTasks, setTypesetTasks] = useState<DtpTask[]>([]);
  const [typesetLoading, setTypesetLoading] = useState(false);
  const [typesetInstruction, setTypesetInstruction] = useState('');
  const [typesetSelectedOp, setTypesetSelectedOp] = useState<DtpOperationType>('correction');
  const [typesetInputFiles, setTypesetInputFiles] = useState<DtpFile[]>([]);
  const [typesetPreview, setTypesetPreview] = useState<string | null>(null);
  const [typesetAdobeStatus, setTypesetAdobeStatus] = useState<'unknown' | 'connected' | 'disconnected'>('unknown');
  ...
  const handleTypesetUpload = async (file: File) => {
    const b64 = await new Promise<string>((res) => {
      const reader = new FileReader();
      reader.onload = () => res((reader.result as string).split(',')[1]);
      reader.readAsDataURL(file);
    });
    const dtpFile: DtpFile = {
      id: crypto.randomUUID(),
      name: file.name,
    ...
    setTypesetInputFiles(prev => [...prev, dtpFile]);

    // PDF preview
    if (file.type === 'application/pdf' || file.name.endsWith('.pdf')) {
      try {
        const apiUrl = (getConfig('VITE_API_URL') || import.meta.env.VITE_API_URL || '').replace(/\/$/, '');
        const res = await fetch(`${apiUrl}/dtp/preview`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (res.ok) {
          ...
          setTypesetPreview(data.preview_png_b64 || null);
          dtpFile.page_count = data.page_count;
          dtpFile.preview_png_b64 = data.preview_png_b64;
          setTypesetInputFiles(prev => prev.map(f => f.id === dtpFile.id ? dtpFile : f));
        }
      } catch { /* preview optional */ }
    }
    flash(`${file.name} を追加しました`, 'ok');
  };

  const handleTypesetFromDrive = async () => {
    try {
      const file = await pickFileFromDrive();
      if (file) handleTypesetUpload(file);
    } catch (e: any) {
      flash(e.message || 'Google Drive接続エラー', 'error');
    }
  };

  const handleTypesetExecute = async () => {
    if (typesetInputFiles.length === 0) { flash('ファイルを選択してください', 'error'); return; }
    setTypesetLoading(true);
    flash('処理中…', 'info');

    const task: DtpTask = {
      id: crypto.randomUUID(),
      name: typesetInputFiles[0]?.name || '新規タスク',
      status: 'processing',
      operation: typesetSelectedOp,
      input_files: typesetInputFiles,
      output_files: [],
      params: {},
      correction_instructions: typesetInstruction || undefined,
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    };
    setTypesetTasks(prev => [task, ...prev]);

    try {
      const apiUrl = (getConfig('VITE_API_URL') || import.meta.env.VITE_API_URL || '').replace(/\/$/, '');

      if (typesetSelectedOp === 'correction' && typesetInstruction) {
        // AI原稿修正
        const res = await fetch(`${apiUrl}/dtp/correct`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            pdf_b64: typesetInputFiles[0]?.data_b64,
            instructions: typesetInstruction,
          }),
        });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();

        const corrections: DtpCorrection[] = (data.corrections || []).map((c: any) => ({
          ...c, applied: true,
        }));

        const updatedTask: DtpTask = {
          ...task,
          status: 'review',
          corrections,
          output_files: data.corrected_pdf_b64 ? [{
            id: crypto.randomUUID(),
            name: `corrected_${task.name}`,
            mime_type: 'application/pdf',
            data_b64: data.corrected_pdf_b64,
            preview_png_b64: data.preview_png_b64,
          }] : [],
          updated_at: new Date().toISOString(),
        };
        setTypesetTasks(prev => prev.map(t => t.id === task.id ? updatedTask : t));
        flash('AI修正案を生成しました', 'ok');
      } else {
        // Adobe操作 or PDF操作
        const res = await fetch(`${apiUrl}/dtp/execute`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            operation: typesetSelectedOp,
            files: typesetInputFiles.map(f => ({ name: f.name, data_b64: f.data_b64 })),
            params: { instruction: typesetInstruction },
          }),
        });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
      ...
      setTypesetTasks(prev => prev.map(t => t.id === task.id ? updatedTask : t));
      flash(`処理エラー: ${e.message}`, 'error');
    } finally {
      setTypesetLoading(false);
    }
  };

  const typesetOpLabels: Record<DtpOperationType, { label: string; icon: typeof Layers; desc: string }> = {
    correction: { label: '原稿修正', icon: Wand2, desc: 'AIが修正箇所を検出・自動修正' },
    ps_edit: { label: 'Photoshop編集', icon: FileText, desc: '画像加工・テキスト修正' },
    id_layout: { label: 'InDesign組版', icon: Layers, desc: '自動レイアウト・配置調整' },
    ai_vector: { label: 'Illustrator', icon: FileText, desc: 'ベクター・ロゴ修正' },
    pdf_merge: { label: 'PDF結合', icon: Plus, desc: '複数PDFを1つに結合' },
    pdf_split: { label: 'PDF分割', icon: FileText, desc: 'ページを個別PDFに分割' },
    pdf_compress: { label: 'PDF圧縮', icon: Download, desc: 'ファイルサイズを最適化' },
    pdf_ocr: { label: 'PDF OCR', icon: Eye, desc: 'テキスト認識・検索可能化' },
  };

  const statusColors: Record<string, { bg: string; text: string; label: string }> = {
    pending: { bg: '#f1f5f9', text: '#64748b', label: '待機中' },
    processing: { bg: '#dbeafe', text: '#2563eb', label: '処理中' },
    review: { bg: '#fef3c7', text: '#d97706', label: 'レビュー' },
    completed: { bg: '#d1fae5', text: '#059669', label: '完了' },
    failed: { bg: '#fee2e2', text: '#dc2626', label: 'エラー' },
  };

  // ── Typeset render functions ──

  const renderTypesetList = () => (
    <div className="flex-1 overflow-y-auto p-6">
      <div className="flex items-center gap-3 mb-6">
        <button
          onClick={() => setView(AppState.TYPESET_AI)}
          className="text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors shadow-sm hover:opacity-90"
          style={{ background: C.accent }}
        >
          <Plus size={16} /> 新規タスク
        ...
        <button
          onClick={async () => {
            try {
              const apiUrl = (getConfig('VITE_API_URL') || import.meta.env.VITE_API_URL || '').replace(/\/$/, '');
              const res = await fetch(`${apiUrl}/dtp/status`);
              if (res.ok) {
                const data = await res.json();
                setTypesetAdobeStatus(data.proxy_connected ? 'connected' : 'disconnected');
                flash(`Adobe: ${data.proxy_connected ? '接続中' : '未接続'} / PyMuPDF: ${data.pymupdf}`, 'info');
              }
            } catch { setTypesetAdobeStatus('disconnected'); flash('バックエンド未接続', 'error'); }
          }}
          className="px-3 py-2 rounded-lg text-sm font-medium border flex items-center gap-2"
          style={{ borderColor: C.border, color: C.textSec }}
        >
          <RefreshCw size={14} /> 接続確認
        </button>
        {typesetAdobeStatus !== 'unknown' && (
          <span className="text-xs px-2 py-1 rounded-full" style={{
            background: typesetAdobeStatus === 'connected' ? '#d1fae5' : '#fee2e2',
            color: typesetAdobeStatus === 'connected' ? '#059669' : '#dc2626',
          }}>
            Adobe {typesetAdobeStatus === 'connected' ? '接続中' : '未接続'}
          </span>
        )}
      </div>
      ...
      {typesetLoading && (
        <div className="flex items-center gap-3 mb-4 p-4 rounded-lg" style={{ background: C.accentBg }}>
          <div className="w-5 h-5 border-2 border-teal-300 border-t-teal-600 rounded-full animate-spin" />
          <span className="text-sm font-medium" style={{ color: C.accent }}>処理中…</span>
        </div>
      )}

      {typesetTasks.length === 0 && !typesetLoading ? (
        <div className="text-center py-20" style={{ color: C.muted }}>
          <Layers size={48} className="mx-auto mb-4 opacity-50" />
          <p className="text-lg font-medium mb-2">組版タスクがありません</p>
          <p className="text-sm">「新規タスク」からPDFや画像をアップロードしてください</p>
        </div>
      ) : (
        <div className="space-y-3">
          {typesetTasks.map(task => {
            const sc = statusColors[task.status] || statusColors.pending;
            const opInfo = typesetOpLabels[task.operation];
            return (
              <div key={task.id} className="p-4 rounded-xl border cursor-pointer hover:shadow-sm transition-shadow" style={{ background: C.card, borderColor: C.border }}>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    {task.input_files[0]?.preview_png_b64 && (
                      <img src={`data:image/png;base64,${task.input_files[0].preview_png_b64}`} className="w-12 h-12 rounded object-cover border" style={{ borderColor: C.border }} />
                    )}
                    <div>
                      <h3 className="font-medium text-sm" style={{ color: C.text }}>{task.name}</h3>
                      <p className="text-xs" style={{ color: C.muted }}>{new Date(task.created_at).toLocaleString('ja-JP')}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] px-2 py-0.5 rounded-full font-medium" style={{ background: C.surface, color: C.accent }}>
                      {opInfo?.label || task.operation}
                    </span>
                ...
                {task.correction_instructions && (
                  <p className="text-xs mt-2 p-2 rounded-lg truncate" style={{ background: C.surface, color: C.textSec }}>
                    {task.correction_instructions}
                  </p>
                )}
                {task.corrections && task.corrections.length > 0 && (
                  <div className="mt-2 flex items-center gap-2">
                    <span className="text-[10px] font-medium" style={{ color: C.accent }}>{task.corrections.length}件の修正提案</span>
                  </div>
                )}
                {task.error_message && (
                  <p className="text-xs mt-2 text-red-500">{task.error_message}</p>
                )}
                {task.output_files.length > 0 && task.output_files[0]?.data_b64 && (
                  <div className="mt-3 flex gap-2">
                    <a
                      href={`data:application/pdf;base64,${task.output_files[0].data_b64}`}
                      download={task.output_files[0].name || 'output.pdf'}
  ...
  const renderTypesetHistory = () => (
    <div className="flex-1 overflow-y-auto p-6">
      {typesetTasks.filter(t => t.status === 'completed' || t.status === 'failed').length === 0 ? (
        <div className="text-center py-20" style={{ color: C.muted }}>
          <Clock size={48} className="mx-auto mb-4 opacity-50" />
          <p className="text-lg font-medium">履歴はまだありません</p>
        </div>
        ...
        <div className="space-y-2">
          {typesetTasks
            ...
            .map(task => {
              const sc = statusColors[task.status] || statusColors.pending;
              return (
                <div key={task.id} className="flex items-center gap-4 p-3 rounded-lg border" style={{ background: C.card, borderColor: C.border }}>
                  <Layers size={18} style={{ color: C.accent }} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate" style={{ color: C.text }}>{task.name}</p>
                    <p className="text-xs" style={{ color: C.muted }}>
                      {task.completed_at ? new Date(task.completed_at).toLocaleString('ja-JP') : new Date(task.updated_at).toLocaleString('ja-JP')}
                    </p>
                  </div>
                  <span className="text-[10px] px-2 py-0.5 rounded-full font-medium" style={{ background: sc.bg, color: sc.text }}>
                    {sc.label}
  ...
  const renderTypesetAI = () => (
    <div className="flex-1 overflow-y-auto p-6">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-8">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: C.accentBg }}>
            <Layers size={32} style={{ color: C.accent }} />
          </div>
          <h3 className="text-lg font-bold mb-2" style={{ color: C.text }}>AI自動組版</h3>
          <p className="text-sm" style={{ color: C.muted }}>
            原稿PDFをアップロードし、修正指示を入力するだけで<br />
            AIが修正箇所を検出、Adobe連携で自動修正します
          </p>
        </div>

        {/* Operation selector */}
        <div className="mb-6">
          <h4 className="text-xs font-bold mb-3 uppercase tracking-wider" style={{ color: C.muted }}>操作タイプ</h4>
          <div className="grid grid-cols-4 gap-2">
            {(Object.keys(typesetOpLabels) as DtpOperationType[]).map(op => {
              const info = typesetOpLabels[op];
              const active = typesetSelectedOp === op;
              return (
                <button
                  key={op}
                  onClick={() => setTypesetSelectedOp(op)}
                  className="p-3 rounded-xl border text-left transition-all"
                  style={{
                    background: active ? C.accentBg : C.card,
                    borderColor: active ? C.accent : C.border,
                    ...(active ? { boxShadow: `0 0 0 1px ${C.accent}` } : {}),
                  }}
                >
                  <info.icon size={18} style={{ color: active ? C.accent : C.muted }} className="mb-1.5" />
                  <p className="text-xs font-bold" style={{ color: active ? C.accent : C.text }}>{info.label}</p>
                  <p className="text-[10px] mt-0.5" style={{ color: C.muted }}>{info.desc}</p>
                </button>
              );
            })}
          </div>
        ...
        {/* File upload */}
        <div className="mb-6">
          <h4 className="text-xs font-bold mb-3 uppercase tracking-wider" style={{ color: C.muted }}>入力ファイル</h4>
          <div className="flex gap-3 mb-3">
            <button
              onClick={handleTypesetFromDrive}
              className="flex items-center gap-2 px-4 py-3 rounded-xl border-2 border-dashed hover:border-teal-300 transition-colors flex-1"
              style={{ borderColor: C.border, background: C.surface }}
            >
              <HardDrive size={20} style={{ color: C.accent }} />
              <div className="text-left">
                <span className="text-sm font-medium block" style={{ color: C.text }}>Google Driveから選択</span>
                <span className="text-[10px]" style={{ color: C.muted }}>PDF・画像・AIファイル対応</span>
              </div>
            </button>
          </div>
          {typesetInputFiles.length > 0 && (
            <div className="space-y-2">
              {typesetInputFiles.map(f => (
                <div key={f.id} className="flex items-center gap-3 p-2 rounded-lg border" style={{ background: C.card, borderColor: C.border }}>
                  {f.preview_png_b64 ? (
                    <img src={`data:image/png;base64,${f.preview_png_b64}`} className="w-10 h-10 rounded object-cover" />
                  ) : (
                    <FileText size={20} style={{ color: C.muted }} />
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate" style={{ color: C.text }}>{f.name}</p>
                    <p className="text-[10px]" style={{ color: C.muted }}>
                      {f.size_bytes ? `${(f.size_bytes / 1024).toFixed(0)} KB` : ''}
                      {f.page_count ? ` / ${f.page_count}ページ` : ''}
                    </p>
                  </div>
                  <button onClick={() => setTypesetInputFiles(prev => prev.filter(x => x.id !== f.id))} className="text-slate-400 hover:text-red-500">
                    <Trash2 size={14} />
                  </button>
                </div>
              ))}
        ...
        {/* Instruction input */}
        {(typesetSelectedOp === 'correction' || typesetSelectedOp === 'ps_edit' || typesetSelectedOp === 'id_layout' || typesetSelectedOp === 'ai_vector') && (
          <div className="mb-6">
            <h4 className="text-xs font-bold mb-3 uppercase tracking-wider" style={{ color: C.muted }}>修正指示</h4>
            <textarea
              value={typesetInstruction}
              onChange={e => setTypesetInstruction(e.target.value)}
              placeholder="例: p.3の電話番号を修正: 03-1234-5678 → 03-9876-5432&#10;例: ヘッダーのフォントサイズを14ptに変更&#10;例: 会社ロゴを右上に移動"
              rows={4}
              className="w-full p-4 rounded-xl border text-sm resize-none focus:outline-none focus:ring-2 focus:ring-teal-200"
              style={{ borderColor: C.border, background: C.surface }}
            />
          </div>
        )}
        ...
        <div className="flex justify-center mb-8">
          <button
            onClick={handleTypesetExecute}
            disabled={typesetLoading || typesetInputFiles.length === 0}
            className="text-white px-8 py-3 rounded-xl text-sm font-bold flex items-center gap-2 shadow-lg hover:opacity-90 transition-opacity disabled:opacity-50"
            style={{ background: C.accent }}
          >
            {typesetLoading ? (
              <>
        ...
        {/* Adobe connection info */}
        <div className="p-4 rounded-xl border" style={{ background: C.card, borderColor: C.border }}>
          <h4 className="text-xs font-bold mb-3 uppercase tracking-wider" style={{ color: C.muted }}>Adobe連携</h4>
          <div className="space-y-2">
            {['Photoshop (テキスト・画像編集)', 'InDesign (レイアウト・組版)', 'Illustrator (ベクター・ロゴ)'].map(m => (
              <div key={m} className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full" style={{ background: C.accent }} />
                <span className="text-sm" style={{ color: C.textSec }}>{m}</span>
              </div>
          ...
          <p className="text-[10px] mt-3" style={{ color: C.muted }}>
            Adobe操作にはプロキシサーバー (localhost:3001) + UXPプラグインが必要です
          </p>
        </div>
      </div>
    </div>
