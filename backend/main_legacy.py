...
あなたは印刷・DTPの専門家です。この画像から「すべてのテキスト要素」および「ロゴ内のテキスト」を正確に抽出してください。
対象は名刺、パンフレット、チラシ、請求書など様々なタイプの可能性があります。

【抽出ルール】
1. 画像上に表示されているすべてのテキスト（文字）を**一切の漏れなく**抽出すること。
   ※ 小さな文字、電話番号、FAX番号、メールアドレス、URL、住所、郵便番号、注記、ページ番号はもちろんのこと、
   ※ 「ロゴマークの中に含まれる文字（例：会社名の英字ロゴなど）」や「透かし文字」も必ずテキストとして抽出してください。
2. テキスト要素の位置を画像全体を100x100として正規化した座標で正確に返すこと。
3. テキストが改行で分かれている場合でも、一つの論理的なブロック（同じフォント・サイズで隣接するもの）は1つの要素として返すこと。
4. 縦書きテキストも正確に検出し、writing_direction を "vertical" にすること。
5. テーブル内のテキストは、セルごと・行ごとに分けて抽出すること。
6. 単語の途中の不自然なスペースは除去すること。
    ...
    try:
        ...
        for i in range(len(pdf_doc)):
            ...
            if use_docai and i < len(docai_results):
                ...
                # ALWAYS run Gemini to catch missing vector logos/text in embedded PDFs!
                print(f"Page {i}: Vector PDF detected, augmenting PyMuPDF with Gemini Vision")
                active_gemini_key = x_gemini_api_key or GEMINI_API_KEY
                if active_gemini_key:
                    try:
                        gemini_spans = _extract_spans_gemini(
                        ...
                        print(f"Page {i}: Gemini補完スキップ: {err}")

                # コンテキストベースのスパン統合
                spans = _merge_context_spans(spans, rect.width, rect.height)
                if spans:
                    print(f"Page {i}: PyMuPDF + Gemini hybrid → {len(spans)} spans")
            else:
